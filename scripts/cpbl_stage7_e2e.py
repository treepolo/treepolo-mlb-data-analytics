...
async def response_for_click(page, selector: str) -> dict[str, Any]:
    async with page.expect_response(
        lambda r: r.url.endswith("/api/analyze") and r.request.method == "POST",
        timeout=120_000,
    ) as pending:
...
async def run_browser(root: Path, base_url: str) -> dict[str, Any]:
    ...
    reference = backend["reference_fastball_type"]
    ...
    async with async_playwright() as pw:
        ...
        await page.wait_for_function(
            "() => window.treepoloDatasetReady && !!window.treepoloFieldCatalog?.fields?.().length",
            timeout=60_000,
        )
        ...
        async def q1():
            ...
            for arrangement in ("consecutive", "none_adjacent"):
                ...
                if count <= 0:
                    raise AssertionError(f"#1 UI {arrangement} returned no rows")
                ...
                "stages": [
                    {
                        ...
                        "event": {"field": "pitch_type", "op": "eq", "value": target},
                        "occurrence": 3,
                        "exact_count": 3,
            ...
            if not {"consecutive", "none_adjacent"}.issubset(cohorts):
                raise AssertionError("#1 UI workflow lacks both cohorts")
            return {
                "reference_fastball_type": reference,
                "target": target,
                "classic_counts": counts,
                "workflow_rows": body.get("row_count"),
            }
        ...
        async def q2():
            ...
            if not arsenal.get("rows"):
                raise AssertionError("#2 Arsenal UI empty")
            ...
            if not role.get("rows"):
                raise AssertionError("#2 Pitch Role UI empty")
            payload = {
                "mode": "workflow",
                "stages": [
                    {
            ...
            await apply_workflow(page, payload)
            wf = await response_for_click(page, "#workflow-panel button.primary")
            if not wf.get("rows"):
                raise AssertionError("#2 same-arsenal workflow UI empty")
            return {
                "arsenal_rows": arsenal.get("row_count"),
                "role_rows": role.get("row_count"),
                "workflow_rows": wf.get("row_count"),
            }
        ...
        async def q3():
            ...
            if not body.get("rows"):
                raise AssertionError("#3 UI workflow empty")
        ...
        async def q4():
                    ...
                    },
                    {
                        ...
                        "metrics": [
                            ...
                            },
                            {
                                ...
                                "condition": {"field": "pitch_type", "op": "eq", "value": reference},
                            },
                            {
                                "function": "avg",
                                "field": "release_speed",
            ...
            body = await response_for_click(page, "#workflow-panel button.primary")
            if not body.get("rows"):
                raise AssertionError("#4 UI workflow empty")
            return {"reference": reference, "rows": body.get("row_count")}
        ...
        async def q5():
            ...
            body = await response_for_click(page, "#workflow-panel button.primary")
            if not body.get("rows"):
                raise AssertionError("#5 UI workflow empty")
            return {"reference": reference, "rows": body.get("row_count")}
        ...
        async def q6():
            ...
            await set_condition(page, "#follow-between", "pitch_type", "eq", reference)
            await page.locator("#follow-gap").fill("3")
            body = await response_for_click(page, '[data-run="follow_event"]')
            if not body.get("rows"):
                raise AssertionError("#6 UI follow-event empty")
            return {"reference": reference, "target": target, "rows": body.get("row_count")}

        async def q7():
            await activate(page, "cross-panel")
            await add_classic_filter(page, "cross", "pitch_type", "eq", reference)
            ...
            if not body.get("rows"):
                raise AssertionError("#7 UI cross-level empty")
            return {"reference": reference, "rows": body.get("row_count")}
        ...
        async def q8():
            ...
            if count <= 0:
                raise AssertionError("#8 UI arsenal-change empty")
            return {"rows": count}
        ...
        async def q9():
            ...
            if not body.get("rows"):
                raise AssertionError("#9 UI percentile empty")
        ...
        async def q10():
            ...
            payload = {
                ...
                "entity_fields": ["pitcher"],
                "min_usage": 0.05,
                "reference_pitch_type": reference,
            ...
            if not ok:
                raise AssertionError("#10 cluster UI refused payload")
            ...
            if not comparison.get("rows"):
                raise AssertionError("#10 UI cluster comparison empty")
            return {"reference": reference, "rows": comparison.get("row_count"), "features": d["features"]}
...
def main() -> None:
    ...
    config_path = root / "stage7_e2e_config.json"
    ...
    try:
        ...
        if report["failed"]:
            raise SystemExit("Stage 7G browser E2E failed: " + ",".join(map(str, report["failed"])))
    finally:
        proc.terminate()
        try:
