...
def engine_for(config: AppConfig, *, root_override: Path | None = None) -> tuple[Any, CPBLSyncEngine]:
    spec = dataset_spec(config, "cpbl")
    if root_override is not None:
        local_config = config_for(root_override, config.analysis_backend)
        spec = dataset_spec(local_config, "cpbl")
        config = local_config
    client = CPBLClient(
...
def copy_raw(src_root: Path, dst_root: Path) -> None:
    src = src_root / "raw"
    dst = dst_root / "raw"
    if dst.exists():
        shutil.rmtree(dst)
...
def lifecycle(root: Path) -> None:
    ...
    first_game = next(iter(archive.iter_games()), None)
    ...
    interrupted = CPBLSyncEngine(resume_spec.database_path, resume_spec.raw_root, FailOneDate(delegate, date(2026,5,11)))
    interrupted_stats = interrupted.backfill(date(2026,5,10), date(2026,5,11), continue_on_error=True, resume=False)
    resumed = CPBLSyncEngine(resume_spec.database_path, resume_spec.raw_root, delegate)
    resumed_stats = resumed.backfill(date(2026,5,10), date(2026,5,11), continue_on_error=False, resume=True)
    ...
    report = {
        ...
        "before": before,
        "after_rebuild": after,
        "first_rebuild": vars(first_rebuild),
        "second_rebuild": vars(second_rebuild),
        "stable_day_first": vars(stable_first),
        "stable_day_second": vars(stable_second),
        "interrupted": vars(interrupted_stats),
        "resumed": vars(resumed_stats),
    ...
    if stable_second.inserted or stable_second.updated:
        blockers.append(f"repeat stable-day ingest changed rows: {vars(stable_second)}")
    ...
    if scheduler_called["count"] != 1:
        blockers.append("scheduler --once path did not invoke one update")
    if blockers:
...
def _canon(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {k: _canon(v) for k, v in sorted(value.items()) if k not in {"backend"}}
...
def _pitch_types(conn: sqlite3.Connection) -> list[str]:
    return [str(r[0]) for r in conn.execute("SELECT pitch_type FROM pitches WHERE pitch_type IS NOT NULL GROUP BY pitch_type ORDER BY COUNT(*) DESC")]


def representative_payloads(path: Path) -> list[tuple[str, dict[str, Any]]]:
    conn = sqlite3.connect(path)
    pitch_types = _pitch_types(conn)
    ...
    return [
        ("basic", {"mode":"basic","filters":filters,"group_by":["pitch_type"],"metrics":[{"function":"count"},{"function":"avg","field":"release_speed"}],"result_sort":[{"field":"row_count","descending":True}],"limit":5000}),
        ("sequence", {"mode":"sequence_pattern","filters":filters,"event":{"field":"pitch_type","op":"eq","value":target},"occurrence":1,"arrangement":"any"}),
        ("follow", {"mode":"follow_event","filters":filters,"anchor":{"field":"pitch_type","op":"eq","value":target},"target":{"field":"pitch_type","op":"eq","value":target},"between":[{"field":"pitch_type","op":"eq","value":"FF"}],"max_gap":3}),
        ("arsenal", {"mode":"arsenal","filters":filters,"entity_fields":["pitcher"],"min_usage":0.05,"tie_method":"dense_rank"}),
        ("role", {"mode":"pitch_role","filters":filters,"entity_fields":["pitcher"],"metric_kind":"usage_rate","rank":1,"descending":True,"exclude_pitch_types":["FF"],"tie_method":"row_number"}),
        ("temporal", {"mode":"temporal","filters":filters,"entity_fields":["pitcher","pitch_type"],"period_field":"game_pk","value_field":"release_speed","function":"avg","direction":"previous","offset":1}),
        ("percentile", {"mode":"percentile","filters":filters,"entity_fields":["pitcher"],"value_field":"release_speed","threshold":0.8,"side":"high"}),
        ("cross", {"mode":"cross_level","filters":[*filters,{"field":"pitch_type","op":"eq","value":"FF"}],"unit_fields":["pitcher","game_pk"],"baseline_fields":["pitcher"],"value_field":"release_speed","function":"avg"}),
        ("arsenal_change", {"mode":"arsenal_change","filters":filters,"entity_fields":["pitcher"],"min_usage":0.05,"period_a":{"start":str(min_date),"end":"2026-06-30"},"period_b":{"start":"2026-07-01","end":str(max_date)}}),
        ("workflow", {"mode":"workflow","filters":filters,"stages":[{"kind":"aggregate","group_by":["pitcher","pitch_type"],"metrics":[{"function":"count","alias":"pitch_count"},{"function":"avg","field":"vert_appr_angle","alias":"avg_vaa"}]},{"kind":"rank","partition_by":["pitcher"],"order_by":[{"field":"pitch_count","descending":True}],"method":"dense_rank","alias":"usage_rank"}],"limit":5000}),
    ]
...
def parity(root: Path) -> None:
    ...
    for name, payload in representative_payloads(spec.database_path):
        s = sqlite_facade.analyze(payload)
        d = duck_facade.analyze(payload)
        same = _canon(s) == _canon(d)
        checks.append({"name":name,"same":same,"sqlite_rows":s.get("row_count"),"duckdb_rows":d.get("row_count")})
        if not same:
            failures.append(name)
    report = {"stage":"7E","mirror_full_build":full,"mirror_noop_refresh":noop,"checks":checks}
    dump_json(root / "reports" / "7e_parity.json", report)
    if failures:
...
def performance(root: Path) -> None:
    ...
    t0 = time.perf_counter(); first = services.analyze(payload); first_s = time.perf_counter()-t0
    t0 = time.perf_counter(); second = services.analyze(payload); second_s = time.perf_counter()-t0
    history = services.history(10)
    saved = services.save_analysis({"name":"Stage 7 cache acceptance","notes":"automated Stage 7F","analysis_payload":payload,"cache_key":second.get("cache",{}).get("key"),"data_revision":second.get("cache",{}).get("data_revision")})
    ...
    try:
        startup_s = wait_http("http://127.0.0.1:8877/api/meta", 45)
    finally:
        proc.terminate()
    ...
    report = {
        "stage":"7F",
        "benchmark":bench,
        "cache":{"first_seconds":first_s,"second_seconds":second_s,"second_hit":second.get("cache",{}).get("hit"),"history_items":len(history),"saved_loaded":bool(loaded_saved)},
        "ui_startup_seconds":startup_s,
...
def main() -> None:
    ...
    if args.command == "lifecycle": lifecycle(args.root)
    elif args.command == "parity": parity(args.root)
    else: performance(args.root)
