(() => {
  ...
  function csv(value) {
    return window.treepoloMultiField?.parse?.(value) || String(value || "").split(",").map(v=>v.trim()).filter(Boolean);
  }
  ...
  window.fetch = async function treepoloStage4Fetch(input, init={}) {
    ...
    const response = await upstreamFetch(input, requestInit);
  ...
  function installMetricValidation() {
    ...
    document.addEventListener("click",e=>{
      ...
      const invalid=Array.from(host.querySelectorAll(".metric-row")).find(row=>{syncMetricRow(row);return row.querySelector(".metric-function")?.value!=="count"&&!row.querySelector(".metric-field")?.value;});
      if(!invalid)return;
  ...
  function setControl(id,value,{checked=false}={}) {
    ...
    else if(el.matches?.("input[data-multi-field]"))window.treepoloMultiField?.write?.(el,Array.isArray(value)?value:[value],{emit:false});
    else if(el.multiple){const wanted=new Set(Array.isArray(value)?value:[value]);Array.from(el.options).forEach(option=>{option.selected=wanted.has(option.value);});}
  ...
  function applyLegacyPayload(p) {
    const m=p.mode;
    if(m==="basic"){setFilters("basic",p.filters);setControl("basic-group",p.group_by||[]);setMetrics(p.metrics||[]);setControl("basic-limit",p.limit??200);}
    else if(m==="sequence_pattern"){setFilters("sequence",p.filters);setSingle("sequence-event",p.event);setControl("sequence-occurrence",p.occurrence??1);setControl("sequence-exact",p.exact_count??"");setControl("sequence-arrangement",p.arrangement||"any");setControl("sequence-last",p.require_last_event,{checked:true});}
    else if(m==="follow_event"){setFilters("follow",p.filters);setSingle("follow-anchor",p.anchor);setSingle("follow-target",p.target);setSingle("follow-between",(p.between||[])[0]||{});setControl("follow-gap",p.max_gap??3);}
    else if(m==="arsenal"){setFilters("arsenal",p.filters);setControl("arsenal-entities",p.entity_fields||[]);setControl("arsenal-min-usage",p.min_usage??.05);setControl("arsenal-tie",p.tie_method||"dense_rank");}
    else if(m==="pitch_role"){setFilters("role",p.filters);setControl("role-entities",p.entity_fields||[]);setControl("role-metric-kind",p.metric_kind||"usage_rate");setControl("role-value-field",p.value_field||"release_speed");setControl("role-function",p.function||"avg");setControl("role-rank",p.rank??1);setControl("role-direction",p.descending===false?"asc":"desc");setControl("role-exclude",(p.exclude_pitch_types||[]).join(", "));setControl("role-tie",p.tie_method||"dense_rank");}
    else if(m==="temporal"){setFilters("temporal",p.filters);setControl("temporal-entities",p.entity_fields||[]);setControl("temporal-period",p.period_field||"game_pk");setControl("temporal-value",p.value_field||"release_speed");setControl("temporal-function",p.function||"avg");setControl("temporal-direction",p.direction||"previous");setControl("temporal-offset",p.offset??1);}
    else if(m==="percentile"){setFilters("percentile",p.filters);setControl("percentile-entities",p.entity_fields||[]);setControl("percentile-value",p.value_field||"release_speed");setControl("percentile-threshold",Number(p.threshold??.8)*100);setControl("percentile-side",p.side||"high");}
    else if(m==="cross_level"){setFilters("cross",p.filters);setControl("cross-unit",p.unit_fields||[]);setControl("cross-baseline",p.baseline_fields||[]);setControl("cross-value",p.value_field||"release_speed");setControl("cross-function",p.function||"avg");}
    else if(m==="arsenal_change"){setFilters("arsenal-change",p.filters);setControl("change-entities",p.entity_fields||[]);setControl("change-min-usage",p.min_usage??.05);setControl("change-a-start",p.period_a?.start||"");setControl("change-a-end",p.period_a?.end||"");setControl("change-b-start",p.period_b?.start||"");setControl("change-b-end",p.period_b?.end||"");}
  ...
  function applyPayload(payload) {
    if(STAGE4_MODES.has(payload.mode)){
      if(payload.mode==="clustering"&&document.getElementById("s4-cluster-partitions"))setControl("s4-cluster-partitions",payload.partition_fields||[]);
  ...
  async function loadItem(item,source={}) {
    ...
    lastPayload=item.payload;lastResult=item.result||null;
    window.treepoloLastAnalysis={
      payload:lastPayload,result:lastResult,
      history_id:source.kind==="history"?(source.id||item.id||null):null,
