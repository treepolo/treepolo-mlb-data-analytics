(() => {
  ...
  if (window.__treepoloStage4DFixups) return;
  window.__treepoloStage4DFixups = true;

  const SVG_NS = "http://www.w3.org/2000/svg";
  const COLORS = ["#2f6fad","#c3543f","#3f8a5b","#8a5ba5","#c18a2d","#477f8e","#9a5966","#65728a","#6c8f36","#a76d34"];
  let lastPrepared = null;
  let pendingSavedVisualization = null;
  ...
  function currentSourceFromControls() {
    const kind=$("#viz-source-kind")?.value||"current";
    if(kind==="current"){
      const payload=window.treepoloLastAnalysis?.payload;
      return payload?{kind:"analysis_payload",payload}:null;
    }
    const raw=$("#viz-source-item")?.value;if(!raw)return null;
    return kind==="recent"?{kind,id:raw}:{kind,id:Number(raw)};
  }

  window.fetch = async function treepoloStage4DFixedFetch(input, init={}) {
    const url=typeof input==="string"?input:input?.url||"";
    const method=String(init?.method||"GET").toUpperCase();
    let requestInit=init;
    if(url.includes("/api/visualization/data")&&method==="POST"&&typeof init.body==="string"){
      try{
        const request=JSON.parse(init.body);
        if(pendingSavedVisualization&&request?.source?.kind==="visualization"&&Number(request.source.id)===Number(pendingSavedVisualization.id)){
          if(pendingSavedVisualization.save_mode==="live")request.section=Number(pendingSavedVisualization.section_index||0);
          else request.section=0;
          pendingSavedVisualization=null;
          requestInit={...init,body:JSON.stringify(request)};
        }
      }catch{}
    }
    ...
    try{
      if(method==="GET"&&/\/api\/visualizations\/\d+/.test(url)&&response.ok){
        ...
        if(body?.item)pendingSavedVisualization=body.item;
      }
